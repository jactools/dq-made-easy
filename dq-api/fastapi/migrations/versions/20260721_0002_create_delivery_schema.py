"""create Delivery schema for Canonical Delivery Registry

Creates a dedicated `delivery` schema in the existing dq-db PostgreSQL instance
to serve as the Canonical Delivery Registry per the Solution Design:
Canonical Data Delivery Phase 1.

This avoids spinning up a separate Delivery container while providing clear
separation of concerns through schema boundaries.

Tables created:
  delivery.deliveries            — Core delivery record (DeliveryId, DeliveryTimeEvent)
  delivery.delivery_lifecycle_events — Timeline events for delivery lifecycle tracking
  delivery.delivery_errors        — Error records for failed deliveries
  delivery.delivery_metadata      — Extended metadata (DDN reference, producer info)
...
def upgrade() -> None:
    # Create the Delivery schema (idempotent)
    ...
    op.create_table(
        ...
        sa.Column("producer_system", sa.Text(), nullable=False,
                   comment="Producer system code (e.g., sap, crm, delivery)"),
    ...
    op.create_index("ix_delivery_deliveries_delivery_time_event", "deliveries",
                     ["delivery_time_event"], schema="delivery")
    op.create_index("ix_delivery_deliveries_producer_system", "deliveries",
                     ["producer_system"], schema="delivery")
    op.create_index("ix_delivery_deliveries_data_object_logical_name", "deliveries",
                     ["data_object_logical_name"], schema="delivery")
    op.create_index("ix_delivery_deliveries_delivered_at", "deliveries",
                     ["delivered_at"], schema="delivery")
    op.create_index("ix_delivery_deliveries_status", "deliveries",
                     ["status"], schema="delivery")
    op.create_index("ix_delivery_deliveries_predecessor_time_event", "deliveries",
                     ["predecessor_time_event"], schema="delivery")
    op.create_index("ix_delivery_deliveries_superseded_by_time_event", "deliveries",
                     ["superseded_by_time_event"], schema="delivery")

    # ------------------------------------------------------------------
    # delivery.delivery_lifecycle_events
    # ------------------------------------------------------------------
    op.create_table(
        ...
        sa.Column("triggered_by", sa.Text(), nullable=True,
                   comment="Service or agent that triggered this event (e.g., DTC, DQ, Guard, Delivery)"),
    ...
    op.create_index("ix_delivery_lifecycle_events_delivery_time_event",
                     "delivery_lifecycle_events", ["delivery_time_event"], schema="delivery")
    op.create_index("ix_delivery_lifecycle_events_event_type",
                     "delivery_lifecycle_events", ["event_type"], schema="delivery")
    op.create_index("ix_delivery_lifecycle_events_occurred_at",
                     "delivery_lifecycle_events", ["occurred_at"], schema="delivery")
    ...
    op.create_index("ix_delivery_errors_delivery_time_event",
                     "delivery_errors", ["delivery_time_event"], schema="delivery")
    op.create_index("ix_delivery_errors_severity",
                     "delivery_errors", ["severity"], schema="delivery")
    ...
    op.create_index("ix_delivery_metadata_data_product_id",
                     "delivery_metadata", ["data_product_id"], schema="delivery")
    op.create_index("ix_delivery_metadata_workspace_id",
                     "delivery_metadata", ["workspace_id"], schema="delivery")


def downgrade() -> None:
    op.drop_table("delivery_metadata", schema="delivery")
    op.drop_table("delivery_errors", schema="delivery")
    op.drop_table("delivery_lifecycle_events", schema="delivery")
