...
def test_initiate_gx_run_plan_alias_returns_view(client, auth_headers, monkeypatch: pytest.MonkeyPatch) -> None:
    from app.main import app

    app.dependency_overrides[gx_endpoints.get_validation_artifact_repository] = lambda: object()
    app.dependency_overrides[gx_endpoints.get_validation_run_plan_repository] = lambda: object()
    app.dependency_overrides[gx_endpoints.get_grouped_execution_planner] = lambda: object()

    async def _fake_create_run_plan(**kwargs):
        request_body = kwargs["request_body"]
        return {
            "runPlanId": "run-plan-1",
            "workspaceId": request_body.workspaceId,
            "planningMode": request_body.planningMode,
            "status": "draft",
            "createdAt": "2026-05-22T20:00:00+00:00",
            "updatedAt": "2026-05-22T20:00:00+00:00",
            "versions": [],
            "transitionEvents": [],
        }

    monkeypatch.setattr(gx_endpoints._gx_run_plan_api, "create_run_plan", _fake_create_run_plan)

    response = client.post(
        "/rulebuilder/v1/gx/run-plans/initiate",
    ...
    body = response.json()
    assert body["run_plan_id"] == "run-plan-1"
    assert body["workspace_id"] == "workspace-1"
    assert body["planning_mode"] == "single_suite"
