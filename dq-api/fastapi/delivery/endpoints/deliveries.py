"""Delivery delivery endpoints for the Canonical Delivery Registry.
...
from delivery.dependencies import get_delivery_repository
from metadata_sdk.domain.vo.delivery import DeliveryId
from metadata_sdk.exceptions import DomainValidationError
...
async def register_delivery(
    ...
    repository=Depends(get_delivery_repository),
) -> EmrDeliveryResponseView:
    """Register a new delivery in the Canonical Delivery Registry.

    Generates a delivery_time_event (UUIDv7) if not provided.
    """
    from delivery.domain.entities import DeliveryEntity

    delivery = DeliveryEntity.model_validate(body.model_dump(mode="python"))

    # Validate DeliveryId format using metadata-sdk
    try:
    ...
    # Generate delivery_time_event if not provided using metadata-utils
    if not delivery.delivery_time_event or not delivery.delivery_time_event.strip():
        delivery.delivery_time_event = generate_uuid7()
...
async def record_lifecycle_event(
    ...
    from delivery.domain.entities import DeliveryLifecycleEventEntity

    event = DeliveryLifecycleEventEntity.model_validate(body.model_dump(mode="python"))
...
async def record_error(
    ...
    from delivery.domain.entities import DeliveryErrorEntity

    error = DeliveryErrorEntity.model_validate(body.model_dump(mode="python"))
...
async def upsert_metadata(
    ...
    from delivery.domain.entities import DeliveryMetadataEntity

    metadata = DeliveryMetadataEntity.model_validate(body.model_dump(mode="python"))
