"""Delivery Registry domain entities for the Canonical Delivery Registry.

These entities represent the canonical delivery model per the Solution Design:
Canonical Data Delivery Phase 1. Delivery stores delivery metadata, lifecycle events,
errors, and extended metadata in a dedicated `delivery` schema.

Delivery entities are self-contained and do not import from the DQ API (app.*).
They use types from metadata-sdk for delivery_type and status.
"""
...
from metadata_sdk.domain.models.delivery_types import DeliveryStatus, DeliveryType
...
class DeliveryLifecycleEventEntity(BaseModel):
...
class DeliveryErrorEntity(BaseModel):
...
class DeliveryMetadataEntity(BaseModel):
...
class DeliveryPageEntity(BaseModel):
    """Paginated delivery list for query results."""

    items: list[DeliveryEntity] = Field(default_factory=list)
    total: int = 0
    page: int = 1
    limit: int = 100
