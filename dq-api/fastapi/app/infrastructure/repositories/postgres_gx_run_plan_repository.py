...
from app.infrastructure.orm.models import GxRunPlanRow
from app.infrastructure.orm.models import GxRunPlanTransitionRow
from app.infrastructure.orm.models import GxRunPlanVersionRow
...
def _serialize_transition_event(row: GxRunPlanTransitionRow) -> dict[str, Any]:
    return {
        "id": row.id,
...
class PostgresGxRunPlanRepository(GxRunPlanRepository):
    ...
    async def create_plan(
        ...
        *,
        ...
        with session_scope(self.database_url) as session:
            if session.get(GxRunPlanRow, run_plan_id) is not None:
            ...
            session.add(
                GxRunPlanRow(
                    ...
                    scope_selector_json=scope_selector.model_dump(by_alias=True, exclude_none=True),
            ...
            session.add(
                GxRunPlanVersionRow(
                    ...
                    run_plan_id=run_plan_id,
                    gx_suite_selection_json=gx_suite_selection.model_dump(by_alias=True, exclude_none=True),
                    suite_id=suite_id,
                    suite_version=suite_version,
                    suite_snapshot_json=(
                        suite_snapshot.model_dump(by_alias=True, exclude_none=True) if suite_snapshot is not None else None
                    ),
                    execution_contract_snapshot_json=(
                        execution_contract_snapshot.model_dump(by_alias=True, exclude_none=True)
                        if execution_contract_snapshot is not None
                        else None
                    ),
                    schedule_definition_json=schedule_definition.model_dump(by_alias=True, exclude_none=True),
    ...
    async def get_plan(self, run_plan_id: str) -> GxRunPlanEntity | None:
        with session_scope(self.database_url) as session:
            plan_row = session.get(GxRunPlanRow, run_plan_id)
            if plan_row is None:
                return None
            version_rows = session.execute(
                select(GxRunPlanVersionRow)
                .where(GxRunPlanVersionRow.run_plan_id == run_plan_id)
                .order_by(GxRunPlanVersionRow.created_at.asc())
            ).scalars().all()
            transition_rows = session.execute(
                select(GxRunPlanTransitionRow)
                .where(GxRunPlanTransitionRow.run_plan_id == run_plan_id)
                .order_by(GxRunPlanTransitionRow.occurred_at.asc())
            ).scalars().all()

        return build_gx_run_plan_entity(self._serialize_plan(plan_row, version_rows, transition_rows))
    ...
    async def transition_plan_version(
        ...
        *,
        ...
        with session_scope(self.database_url) as session:
            ...
            version_row = session.get(GxRunPlanVersionRow, run_plan_version_id)
    ...
    async def list_plans(
        ...
        *,
        ...
        with session_scope(self.database_url) as session:
            stmt = select(GxRunPlanRow)
            if workspace_id is not None:
                stmt = stmt.where(GxRunPlanRow.workspace_id == workspace_id)
            if business_key is not None:
                stmt = stmt.where(GxRunPlanRow.business_key == business_key)
            if status is not None:
                stmt = stmt.where(GxRunPlanRow.status == status)
            stmt = stmt.order_by(GxRunPlanRow.updated_at.desc(), GxRunPlanRow.created_at.desc())
            plan_rows = session.execute(stmt).scalars().all()

            plan_ids = [row.id for row in plan_rows]
            version_map: dict[str, list[GxRunPlanVersionRow]] = {plan_id: [] for plan_id in plan_ids}
            transition_map: dict[str, list[GxRunPlanTransitionRow]] = {plan_id: [] for plan_id in plan_ids}
            if plan_ids:
                version_rows = session.execute(
                    select(GxRunPlanVersionRow)
                    .where(GxRunPlanVersionRow.run_plan_id.in_(plan_ids))
                    .order_by(GxRunPlanVersionRow.created_at.asc())
                ).scalars().all()
                ...
                transition_rows = session.execute(
                    select(GxRunPlanTransitionRow)
                    .where(GxRunPlanTransitionRow.run_plan_id.in_(plan_ids))
                    .order_by(GxRunPlanTransitionRow.occurred_at.asc())
                ).scalars().all()
        ...
        rows = [
            build_gx_run_plan_entity(self._serialize_plan(row, version_map.get(row.id, []), transition_map.get(row.id, [])))
            for row in plan_rows
        ]
        if suite_id is not None:
            rows = [
                row for row in rows if any(str(version.get("suiteId") or "") == suite_id for version in row.get("versions", []))
            ]
        return rows
        ...
        *,
        ...
        with session_scope(self.database_url) as session:
            ...
            if previous_active_version_id and previous_active_version_id != run_plan_version_id:
                previous_row = session.get(GxRunPlanVersionRow, previous_active_version_id)
    ...
    async def deactivate_plan(
        ...
        *,
        ...
        plan = await self.get_plan(run_plan_id)
        if plan is None:
            raise RuntimeError("Failed to deactivate GX run plan")
        return plan
    ...
    async def record_plan_dispatch(
        ...
        *,
        ...
        plan = await self.get_plan(run_plan_id)
        if plan is None:
            raise RuntimeError("Failed to record GX run plan dispatch")
        return plan

    @staticmethod
    def _serialize_version(row: GxRunPlanVersionRow, *, current_active_version_id: str | None) -> dict[str, Any]:
        return {
            "runPlanVersionId": row.id,
            "runPlanId": row.run_plan_id,
            "governanceState": str(row.governance_state or "draft"),
            "gxSuiteSelection": dict(row.gx_suite_selection_json or {}),
            "suiteId": row.suite_id,
            "suiteVersion": row.suite_version,
            "suiteSnapshot": dict(row.suite_snapshot_json or {}) if row.suite_snapshot_json is not None else None,
    ...
    def _serialize_plan(
        self,
        row: GxRunPlanRow,
        version_rows: list[GxRunPlanVersionRow],
        transition_rows: list[GxRunPlanTransitionRow],
    ) -> dict[str, Any]:
        versions = [
            self._serialize_version(version_row, current_active_version_id=row.current_active_version_id)
        ...
        return {
            ...
            "scopeSelector": dict(row.scope_selector_json or {}),
            ...
            "versions": versions,
            "transitionEvents": [self._serialize_transition_event(item) for item in transition_rows],
        }
