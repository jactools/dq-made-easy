...
class PostgreSQLConnector:
    provider = "postgresql"
    capabilities = get_connector_registry_entry("postgresql").capabilities
            ...
            return ConnectorDiscoveryResultEntity(
                ...
                errors=(
                    ConnectorErrorEntity(
                        ...
                        code="postgresql_discovery_failed",
                        retryable=True,
    ...
    def _configuration_errors(self, configuration: ConnectorConfigurationEntity) -> list[ConnectorErrorEntity]:
        ...
        if not isinstance(configuration, ConnectorSecureConfigurationEntity):
            errors.append(
                ConnectorErrorEntity(
                    kind="configuration",
                    message="PostgreSQL connector requires ConnectorSecureConfigurationEntity",
                    code="postgresql_secure_configuration_required",
                    field="credentials",
                )
            )
            return errors
        ...
        if self._normalized_text(configuration.provider) != self.provider:
            errors.append(
                ConnectorErrorEntity(
                    kind="configuration",
                    message="PostgreSQL connector only accepts the 'postgresql' provider",
                    code="postgresql_provider_mismatch",
                    field="provider",
                )
            )

        parameters = configuration.parameters
        host = self._normalized_text(parameters.get("host"))
        ...
        if not host:
            errors.append(
                ConnectorErrorEntity(
                    kind="configuration",
                    message="PostgreSQL connector requires 'host'",
                    code="postgresql_missing_host",
                    field="host",
                )
            )
        if not database:
            errors.append(
                ConnectorErrorEntity(
                    kind="configuration",
                    message="PostgreSQL connector requires 'database'",
                    code="postgresql_missing_database",
                    field="database",
                )
            )
        if not username:
            errors.append(
                ConnectorErrorEntity(
                    kind="configuration",
                    message="PostgreSQL connector requires 'username'",
                    code="postgresql_missing_username",
                    field="username",
                )
            )
        if not password:
            errors.append(
                ConnectorErrorEntity(
                    kind="configuration",
                    message="PostgreSQL connector requires a password credential named 'password'",
                    code="postgresql_missing_password",
                    field="credentials",
                )
            )
    ...
    def _build_database_url(self, configuration: ConnectorSecureConfigurationEntity) -> URL:
        parameters = configuration.parameters
        sslmode = self._normalized_text(parameters.get("sslmode"))
        ...
        return URL.create(
            ...
            host=self._normalized_text(parameters.get("host")) or "",
    ...
    def _discover_items(
        ...
        database_name = self._normalized_text(configuration.parameters.get("database")) or "postgresql"
        ...
        exclude_schemas = set(
            self._selected_names(configuration.parameters.get("exclude_schemas"))
        )

        schema_names = [
            name
            for name in sorted(self._normalized_name_list(inspector.get_schema_names()))
            if name not in _SYSTEM_SCHEMAS and not name.startswith("pg_")
        ]
        if include_schemas:
