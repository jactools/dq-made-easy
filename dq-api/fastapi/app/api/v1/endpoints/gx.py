...
from app.api.v1 import gx_browse_api as _gx_browse_api
...
class GxExecutionRunCommentsRequestView(SnakeModel):
    model_config = ConfigDict(from_attributes=True, alias_generator=to_snake_alias, populate_by_name=True)
...
@router.put(
    ...
    responses={
        200: {"description": "GX execution run comments were updated."},
        404: {"description": "GX execution run not found."},
    },
)
async def update_gx_execution_run_comments(
    run_id: str,
    request_body: GxExecutionRunCommentsRequestView,
    repository: GxExecutionRunRepository = Depends(get_gx_execution_run_repository),
) -> GxExecutionRunView:
    updated_run = await repository.update_run_comments(run_id, request_body.comments)
    if updated_run is None:
        raise HTTPException(status_code=404, detail=f"GX execution run '{run_id}' not found")
    return GxExecutionRunView.model_validate(updated_run.model_dump(mode="python", by_alias=True, exclude_none=False))
