...
- **API (FastAPI transport layer)**: `dq-api` is the primary REST surface, proxied under `/api`. OpenAPI docs are generated from the FastAPI app. Key envs: `DQ_API_INTERNAL_URL`, `DQ_API_LOCAL_URL`, and the UI-facing `VITE_API_URL`.
