...
When creating/editing a rule with business terms:

1. **Open Assign Attributes Modal**
  - Modal now shows "Business-term suggestions" below each business term
   - Suggestions auto-appear if catalog is healthy

2. **Browse Suggestions**
   ```
  business term: "amount"
  Business-term suggestions:
   [transaction_amount (Catalog)]  [order_value (Catalog)]
   ```

3. **Click to Apply**
  - Clicking a suggestion auto-fills the business-term mapping dropdown
