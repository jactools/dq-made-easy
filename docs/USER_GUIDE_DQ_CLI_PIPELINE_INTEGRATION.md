...
The repository already ships a CLI entry point named `dq-run-plan` in [dq-cli/pyproject.toml](../dq-cli/pyproject.toml). Its supported actions and arguments are implemented in [dq-cli/dq_cli/run_plan.py](../dq-cli/dq_cli/run_plan.py).
