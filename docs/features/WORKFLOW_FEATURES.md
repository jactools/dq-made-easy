...
Environment foundation: WF-4 should run local smoke and regression orchestration against the dedicated `test` environment defined in [WF-5 Dedicated Environment Contract and Startup Selection](./WF_5_DEDICATED_ENVIRONMENT_CONTRACT.md), rather than overloading the developer `.env` or public deployment env file.
...
- Decision and rationale are captured in [ADR-012](../../architecture/adr/ADR-012-test-automation-tool-selection-and-evidence-strategy.md).
...
Detailed plan: [WF-5 Dedicated Environment Contract and Startup Selection](./WF_5_DEDICATED_ENVIRONMENT_CONTRACT.md)
