...
## Phase 3 — DuckDB lowerer for GX and Soda

### Goal

Support local SQL-style execution for GX and Soda validations using DuckDB as a lightweight embedded runtime.

### Scope

- DuckDB-backed execution for row-level and aggregate validators.
- SQL-shaped validation paths that can be expressed through GX and Soda-compatible query plans.
- Lightweight diagnostics and failure aggregation for local and CI-friendly execution.

### Deliverables

- Add a `duckdb` adapter module under `dq-engine`.
- Implement lowering into DuckDB SQL or a structured SQL plan suitable for GX and Soda integration.
- Add execution and diagnostics support for embedded validation runs.
- Add regression tests for supported and unsupported constructs.

### Risks and mitigations

- Risk: SQL semantics and function availability may differ from Spark or Trino.
- Mitigation: keep the initial supported set conservative and document unsupported constructs explicitly.
