...
4. [x] (ABS1-GX-04) Implement grouped execution.
   ...
   Implemented by the grouped planner and PySpark executor in [API7 Real DQ Rule Execution Milestone](./API_7_REAL_DQ_RULE_EXECUTION_MILESTONE.md) and [API-7 Real DQ Rule Execution](../features/current/API_7_REAL_DQ_RULE_EXECUTION.md), which both batch compatible suites by `dataObjectVersionId` and run grouped batches in one Spark session where possible.
...
6. [x] (ABS1-GX-06) Separate result persistence.
   ...
   Implemented by API-7 Phase 4 persistence in [API_7_REAL_DQ_RULE_EXECUTION_MILESTONE.md](./API_7_REAL_DQ_RULE_EXECUTION_MILESTONE.md) and [API_7_REAL_DQ_RULE_EXECUTION.md](../features/current/API_7_REAL_DQ_RULE_EXECUTION.md), which store run metadata in the rule/result store and row-level violations in a separate exception store scoped to `dataObjectVersionId`.
