...
- [ ] (SEC5-I-W4-02) Update callers, SDKs, and bootstrap logic to trust the origin service certificate. It is not allowed they rely on a terminating proxy.
...
- [ ] (SEC5-I-W5-01) Convert every health check that to use a TLS listener to HTTPS and certificate validation.
- [ ] (SEC5-I-W5-02) Replace `http://127.0.0.1` probes with `https://127.0.0.1` or another validated TLS endpoint. The service must support it, if it doesn't the service must be adjusted accordingly.
- [ ] (SEC5-I-W5-03) Keep HTTP loopback probes only within a container instance. When the service has no TLS listener yet, the service must be adjusted.
...
- [ ] (SEC5-I-AC-03) No health check or smoke check uses `http://` . HTTPS/TLS must be used.
- [ ] (SEC5-I-AC-04) No inter-container runtime call uses plaintext HTTP. HTTPS/TLS must be used.
