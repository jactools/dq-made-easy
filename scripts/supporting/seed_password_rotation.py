...
# Admin password variable names — persisted in stateful volumes.
# Must NOT be rotated unless volumes are destroyed.
_ADMIN_PASSWORD_VARS = frozenset({
    "DQ_DB_PASSWORD",
    "KONG_DB_PASSWORD",
    "OM_DB_PASSWORD",
    "OM_DB_ROOT_PASSWORD",
...
def _should_rotate(name: str, value: str, *, skip_admin: bool = False) -> bool:
    ...
    - Admin passwords when skip_admin=True
    """
    ...
    for prefix in _NO_ROTATE_PREFIXES:
        if name.startswith(prefix):
            return False
    if skip_admin and name in _ADMIN_PASSWORD_VARS:
        return False
    return True
...
def rotate_env_passwords(
    ...
    *,
    ...
    When *skip_admin* is True, admin password variables (persisted in stateful
    volumes) are left unchanged so that passwords in the database continue to
    match.
    ...
    with env_file.open("r", encoding="utf-8") as fh:
        for line in fh:
            ...
            if parsed is None:
                ...
                continue
            key, value = parsed
            if _should_rotate(key, value, skip_admin=skip_admin):
                new_value = generate_password()
                lines.append(f'{key}="{new_value}"')
                rotated_vars.append((key, new_value))
            elif skip_admin and key in _ADMIN_PASSWORD_VARS:
                skipped_admin_vars.append(key)
                lines.append(line.rstrip("\n"))
            else:
    ...
    if skip_admin:
        header.append(
            f"# Skipped {len(skipped_admin_vars)} admin password(s): "
            f"{', '.join(skipped_admin_vars)}"
        )
    header.append("# Do not commit — regenerated on each startup.")
    header.append("")
...
def main() -> None:
    ...
    parser.add_argument("--no-admin-rotate", action="store_true",
                        help="Do NOT rotate admin passwords (they are persisted in stateful volumes)")
    args = parser.parse_args()

    if args.env_file:
        # Env file password rotation mode
        rotate_env_passwords(
            args.env_file, output_dir=args.output_dir,
            skip_admin=args.no_admin_rotate,
        )
