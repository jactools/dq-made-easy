...
ME_CODE="$(curl -sS --cacert "$CURL_CA_BUNDLE" -H "Authorization: Bearer ${LOGIN_TOKEN}" -o "$ME_BODY_FILE" -w "%{http_code}" "$ME_URL")"
if [ "$ME_CODE" != "200" ]; then
...
RULES_CODE="$(curl -sS --cacert "$CURL_CA_BUNDLE" -H "Authorization: Bearer ${LOGIN_TOKEN}" -o "$RULES_BODY_FILE" -w "%{http_code}" "$RULES_URL")"
