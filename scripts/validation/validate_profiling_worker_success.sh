...
# Purpose: Validate profiling worker success path end-to-end.
#
# What it does:
# - Enqueues a profiling request through the internal API service.
# - Polls Postgres for completed status.
...
my_name="validate_profiling_worker_success.sh"
...
print_usage() {
  cat <<'EOF'
Usage: validate_profiling_worker_success.sh
...
Environment overrides:
  ...
  WAIT_SECONDS             Max seconds to wait for completed status
...
profiling_request_id="pr-live-success-${unique}"
job_id="job-live-success-${unique}"
correlation_id="kong-success-correlation-${unique}"
request_id="kong-success-request-${unique}"
tmp_dir="$(mktemp -d /tmp/profiling-worker-success.XXXXXX)"
...
info "$my_name" "=== Profiling Worker Success Validation ==="
...
info "$my_name" "[2/4] Polling Postgres for completed worker status..."
db_row=""
...
for _ in $(seq 1 "$attempts"); do
  ...
  if [[ -n "$db_row" ]]; then
    ...
    if [[ "$row_status" == "completed" && "$row_started" == "t" && "$row_completed" == "t" && "$row_job_id" == "$job_id" && -z "$row_error" ]]; then
      break
    fi
  fi
...
IFS='|' read -r row_id row_status row_started row_completed row_job_id row_error <<< "$db_row"
if [[ "$row_status" != "completed" || "$row_started" != "t" || "$row_completed" != "t" || "$row_job_id" != "$job_id" || -n "$row_error" ]]; then
  error "$my_name" "Profiling request row did not reach the expected completed state"
  echo "$db_row" >&2
  exit 1
fi

info "$my_name" "[3/4] Checking profiling worker logs for the same job ID..."
worker_log_matches="$(profiling_worker_logs "$((WAIT_SECONDS + 15))" | grep -F "Received job from queue: ${job_id}" || true)"
if [[ -z "$worker_log_matches" ]]; then
  error "$my_name" "Worker logs do not contain receipt of ${job_id}"
  exit 1
fi
...
success "$my_name" "The internal API accepted the request and the worker marked it completed with no error message."