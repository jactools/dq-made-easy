#!/usr/bin/env python3
"""Validation helper: detect data-quality threshold breaches and create a functional_violation incident.
...
Flow:
  ...
  3. Extract the violation_count from the run payload.
  4. If violation_count > DQ_VALIDATION_VIOLATION_THRESHOLD (default 0), POST
     /rulebuilder/v1/incidents with incident_kind="functional_violation" and
     create_itsm_ticket=DQ_VALIDATION_CREATE_ITSM_TICKET.
  5. Assert the response is 201 and contains an incident ID.
  6. Exit 0 with an informational note when violations are at or below threshold
     (no ticket needed — this is a valid outcome, not a test failure).
"""
...
WORKFLOW_LABEL = "validate_rule_violation_incident"
...
def _extract_violation_count(run_payload: dict[str, Any]) -> int:
    """Extract a usable violation count from the run payload.

    GX validation failures may not populate the top-level violation_count field.
    In that case, fall back to failed_expectation_count from the result summary,
    then to the diagnostic count.
    """
    raw = _pick(run_payload, "violationCount", "violation_count", default=None)
    if raw not in (None, ""):
        try:
            return int(raw)
    ...
    result_summary = _pick(run_payload, "resultSummary", "result_summary", default={})
    if isinstance(result_summary, dict):
        results = result_summary.get("results") or []
            ...
            for result in results:
                ...
                try:
                    failed_expectation_count += int(
                        _pick(summary, "failedExpectationCount", "failed_expectation_count", default=0)
                    )
                except (TypeError, ValueError):
                    continue
            if failed_expectation_count > 0:
                return failed_expectation_count
...
def _run_failure_code(payload: dict[str, Any]) -> str:
    return str(_pick(payload, "failureCode", "failure_code", default="")).strip().upper()


def _extract_violated_rule_ids(run_payload: dict[str, Any]) -> list[str]:
    """Extract the list of rule IDs that had violations from the run payload."""
    raw = _pick(run_payload, "violatedRuleIds", "violated_rule_ids", default=[])
    if isinstance(raw, list):
        return [str(r) for r in raw if r]
    rule_id = str(_pick(run_payload, "ruleId", "rule_id", default="")).strip()
    if rule_id:
        return [rule_id]
    return []
...
def _create_functional_violation_incident(
    session: requests.Session,
    kong_url: str,
    token: str,
    *,
    ...
    title = f"DQ threshold not met: {violation_count} violation(s) on {scope_id or run_id}"
...
def _main() -> int:
    ...
    try:
        violation_threshold = int(_optional_env("DQ_VALIDATION_VIOLATION_THRESHOLD", "0"))
    except ValueError:
        raise SystemExit(
            f"[{WORKFLOW_LABEL}] DQ_VALIDATION_VIOLATION_THRESHOLD must be an integer."
        )
    ...
    if effective_run_id:
        ...
        if final_status not in _TERMINAL_STATUSES:
            print(
                f"[{WORKFLOW_LABEL}] Run {effective_run_id!r} is still active "
                f"(status={final_status!r}); waiting for terminal state …"
            )
    ...
    final_status = _run_status(run_payload)
    failure_code = _run_failure_code(run_payload)
    violation_count = _extract_violation_count(run_payload)
    violated_rule_ids = _extract_violated_rule_ids(run_payload)
    ...
    print(
        f"[{WORKFLOW_LABEL}] Run {effective_run_id!r}: "
        f"status={final_status!r}  violation_count={violation_count}"
    )

    allowed_validation_failure = final_status == "failed" and failure_code == "GX_VALIDATION_FAILED"
    if final_status != "succeeded" and not allowed_validation_failure:
        raise RuntimeError(
            f"Expected either a succeeded DQ run or GX_VALIDATION_FAILED for functional_violation validation, "
            f"but run {effective_run_id!r} finished with status={final_status!r} "
            f"and failure_code={failure_code!r}."
        )

    # --- Check threshold ---
    if violation_count <= violation_threshold:
        print(
            f"\n[{WORKFLOW_LABEL}] No incident required: "
            f"violation_count={violation_count} is within threshold={violation_threshold}. "
            "DQ threshold met — all good."
        )
        return 0

    # --- Create incident ---
    print(
        f"[{WORKFLOW_LABEL}] Threshold breached ({violation_count} > {violation_threshold}); "
        f"creating functional_violation incident (create_itsm_ticket={create_itsm_ticket}) …"
    )
    result = _create_functional_violation_incident(
    ...
    print(f"  Violations      : {violation_count} (threshold: {violation_threshold})")
    if violated_rule_ids:
        print(f"  Violated rules  : {', '.join(violated_rule_ids)}")
    ...
    print(
        f"\n[{WORKFLOW_LABEL}] PASSED: functional_violation incident created "
        f"for {violation_count} violation(s) on run {effective_run_id!r}."
    )
    return 0
