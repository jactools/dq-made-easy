...
test_browser_frontend_https() {
  ...
  if grep -rq "frontend:" "$REPO_ROOT/docker-compose/"; then
    verbose_output "✓ Frontend service defined"
  fi
  
  # Check frontend uses TLS listener
  if grep -rA10 "frontend:" "$REPO_ROOT/docker-compose/" | grep -q "listen.*443\|ssl" || \
     grep -rq "frontend-https\|frontend.*tls" "$REPO_ROOT/docker-compose/"; then
...
test_browser_support_https() {
  ...
  if grep -rq "zammad-railsserver.*ssl://" "$REPO_ROOT/docker-compose/"; then
...
test_service_api_to_kong() {
  ...
  if grep -rq "kong.*8443\|kong.*https" "$REPO_ROOT/docker-compose/"; then
    verbose_output "✓ Kong admin API exposes HTTPS (8443)"
    log_info "PASS: API to Kong gateway uses TLS"
    return 0
  elif grep -rq "kong.*:8001" "$REPO_ROOT/docker-compose/" | head -1; then
...
test_service_api_to_redis() {
  ...
  if grep -rq "redis.*--tls\|redis.*rediss" "$REPO_ROOT/docker-compose/"; then
...
test_service_api_to_postgres() {
  ...
  if grep -rq "sslmode=require\|sslmode=verify-full" "$REPO_ROOT/docker-compose/" || \
...
test_all_healthchecks_tls() {
  ...
  local http_hc=$(grep -rc "healthcheck:" "$REPO_ROOT/docker-compose/" || true)
  local tls_hc=$(grep -rc "cacert\|ssl.*certificate" "$REPO_ROOT/docker-compose/" || true)
  
  if [ "$http_hc" -gt 0 ]; then
    verbose_output "Found $http_hc healthchecks in compose"
    
    if grep -rq "healthcheck:" "$REPO_ROOT/docker-compose/" | grep -v "cacert\|ssl_verify"; then
...
test_no_http_ports_advertised() {
  ...
  local http_ports=$(grep -rE "ports:|:80[^0-9]|:8080[^0-9]" "$REPO_ROOT/docker-compose/" | wc -l || true)
  
  # Keycloak and Kong historically use HTTP; check if they're intentional
  if grep -rq "keycloak.*8080\|kong.*8000" "$REPO_ROOT/docker-compose/"; then
...
test_no_plaintext_service_urls() {
  ...
  local plaintext=$(grep -rv "^[[:space:]]*#" "$REPO_ROOT/docker-compose/" | \
                    grep "http://" | \
  ...
  if [ "$plaintext" -gt 0 ]; then
    ...
    grep -rv "^[[:space:]]*#" "$REPO_ROOT/docker-compose/" | grep "http://" | head -3 | while read line; do
      verbose_output "  $line"
    done
    return 0
  fi
