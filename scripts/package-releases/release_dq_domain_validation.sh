...
# Purpose: Build, publish, and version-bump the standalone dq-made-easy-domain-validation package.
#
# What it does:
# - Builds wheel and sdist artifacts for dq-made-easy-domain-validation.
# - Publishes the artifacts to a named Twine repository.
# - Bumps dq-domain-validation/pyproject.toml to the next patch version after a successful publish.
#
# Version: 1.0.0
...
DIST_DIR="${ROOT_DIR}/tmp/dq-domain-validation-release"
REPOSITORY="pypi"
DRY_RUN="false"

usage() {
  cat <<'EOF'
Usage: scripts/package-releases/release_dq_domain_validation.sh [--repository NAME] [--dry-run]
...
require_cmd() {
  ...
  if ! command -v "$name" >/dev/null 2>&1; then
    printf 'release_dq_domain_validation.sh: missing required command: %s\n' "$name" >&2
    exit 2
  fi
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --repository)
      if [[ $# -lt 2 ]]; then
        printf 'release_dq_domain_validation.sh: --repository requires a value\n' >&2
        usage
        exit 2
      fi
      REPOSITORY="$2"
    ...
    -h|--help)
      ...
      ;;
    *)
      printf 'release_dq_domain_validation.sh: unknown argument: %s\n' "$1" >&2
      usage
      exit 2
      ;;
  esac
...
if ! "$PYTHON_RUNNER" --python-bin "$PYTHON_BIN" -m build --version >/dev/null 2>&1; then
  printf 'release_dq_domain_validation.sh: build is required in the repo venv\n' >&2
  exit 2
fi

if ! "$PYTHON_RUNNER" --python-bin "$PYTHON_BIN" -m twine --version >/dev/null 2>&1; then
  printf 'release_dq_domain_validation.sh: twine is required in the repo venv\n' >&2
  exit 2
fi
...
printf 'Building dq-made-easy-domain-validation %s\n' "$CURRENT_VERSION"
...
if [[ "$DRY_RUN" == "true" ]]; then
  printf 'Dry run complete for dq-made-easy-domain-validation %s\n' "$CURRENT_VERSION"
  exit 0
fi

printf 'Publishing dq-made-easy-domain-validation %s to %s\n' "$CURRENT_VERSION" "$REPOSITORY"
