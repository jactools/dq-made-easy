...
#
# What it does:
# - Builds wheel and sdist artifacts for dq-made-easy-cli from the dq-cli project.
...
DIST_DIR="${ROOT_DIR}/tmp/dq-made-easy-cli-release"
REPOSITORY="pypi"
DRY_RUN="false"
...
if ! "$PYTHON_RUNNER" --python-bin "$PYTHON_BIN" -m build --version >/dev/null 2>&1; then
  printf 'release_dq_made_easy_cli.sh: build is required in the repo venv\n' >&2
  exit 2
fi

if ! "$PYTHON_RUNNER" --python-bin "$PYTHON_BIN" -m twine --version >/dev/null 2>&1; then
  printf 'release_dq_made_easy_cli.sh: twine is required in the repo venv\n' >&2
  exit 2
fi
...
printf 'Building dq-made-easy-cli %s\n' "$CURRENT_VERSION"
...
printf 'Publishing dq-made-easy-cli %s to %s\n' "$CURRENT_VERSION" "$REPOSITORY"
