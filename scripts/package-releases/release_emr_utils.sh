...
# Purpose: Build, publish, and version-bump the emr-utils package.
#
# What it does:
# - Delegates wheel build/publish to scripts/release_python_package.sh.
# - Bumps emr-utils/pyproject.toml to the next patch version after a successful publish.
#
# Version: 1.0.0
...
usage() {
  cat <<'EOF'
Usage: scripts/package-releases/release_emr_utils.sh [--repository NAME] [--dry-run]
...
require_cmd() {
  ...
  if ! command -v "$name" >/dev/null 2>&1; then
    printf 'release_emr_utils.sh: missing required command: %s\n' "$name" >&2
    exit 2
  fi
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --repository)
      if [[ $# -lt 2 ]]; then
        printf 'release_emr_utils.sh: --repository requires a value\n' >&2
        usage
        exit 2
      fi
      REPOSITORY="$2"
    ...
    -h|--help)
      ...
      ;;
    *)
      printf 'release_emr_utils.sh: unknown argument: %s\n' "$1" >&2
      usage
      exit 2
      ;;
  esac
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  printf 'Dry run complete for emr-utils %s\n' "$CURRENT_VERSION"
  exit 0
fi
...
printf 'Bumped emr-utils/pyproject.toml from %s to %s\n' "$CURRENT_VERSION" "$NEXT_VERSION"
