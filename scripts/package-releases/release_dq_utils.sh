...
# Purpose: Build, publish, and version-bump the standalone dq-made-easy-utils package.
#
# What it does:
# - Builds wheel and sdist artifacts for dq-made-easy-utils.
# - Publishes the artifacts to a named Twine repository.
# - Bumps dq-utils/pyproject.toml to the next patch version after a successful publish.
#
# Version: 1.0.0
...
DIST_DIR="${ROOT_DIR}/tmp/dq-utils-release"
REPOSITORY="pypi"
DRY_RUN="false"

usage() {
  cat <<'EOF'
Usage: scripts/package-releases/release_dq_utils.sh [--repository NAME] [--dry-run]
...
require_cmd() {
  ...
  if ! command -v "$name" >/dev/null 2>&1; then
    printf 'release_dq_utils.sh: missing required command: %s\n' "$name" >&2
    exit 2
  fi
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --repository)
      if [[ $# -lt 2 ]]; then
        printf 'release_dq_utils.sh: --repository requires a value\n' >&2
        usage
        exit 2
      fi
      REPOSITORY="$2"
    ...
    -h|--help)
      ...
      ;;
    *)
      printf 'release_dq_utils.sh: unknown argument: %s\n' "$1" >&2
      usage
      exit 2
      ;;
  esac
...
if ! "$PYTHON_RUNNER" --python-bin "$PYTHON_BIN" -m build --version >/dev/null 2>&1; then
  printf 'release_dq_utils.sh: build is required in the repo venv\n' >&2
  exit 2
fi

if ! "$PYTHON_RUNNER" --python-bin "$PYTHON_BIN" -m twine --version >/dev/null 2>&1; then
  printf 'release_dq_utils.sh: twine is required in the repo venv\n' >&2
  exit 2
fi
...
printf 'Building dq-made-easy-utils %s\n' "$CURRENT_VERSION"
...
if [[ "$DRY_RUN" == "true" ]]; then
  printf 'Dry run complete for dq-made-easy-utils %s\n' "$CURRENT_VERSION"
  exit 0
fi

printf 'Publishing dq-made-easy-utils %s to %s\n' "$CURRENT_VERSION" "$REPOSITORY"
...
printf 'Bumped dq-utils/pyproject.toml from %s to %s\n' "$CURRENT_VERSION" "$NEXT_VERSION"
