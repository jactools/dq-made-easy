...
usage() {
  cat <<'EOF'
Usage: scripts/package-releases/release_dq_airflow_operator.sh [--repository NAME] [--dry-run]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --repository)
      if [[ $# -lt 2 ]]; then
        printf 'release_dq_airflow_operator.sh: --repository requires a value\n' >&2
        usage
        exit 2
      fi
      REPOSITORY="$2"
    ...
    -h|--help)
      ...
      ;;
    *)
      printf 'release_dq_airflow_operator.sh: unknown argument: %s\n' "$1" >&2
      usage
      exit 2
      ;;
  esac
