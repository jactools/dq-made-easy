...
- The React UI converts received snake_case fields to camelCase for UI use. When creating a payload, the front-end serializes the UI model back to snake_case before sending it to the API.
...
- Treat snake_case as the on-the-wire contract. The backend keeps request bodies untouched and validates snake_case directly at the route/repository boundary.
- The UI must convert at the boundary and keep internal UI models in camelCase.
...
- For page and dialog styling, use the shared semantic tokens in `dq-ui/src/themes.css` such as `--app-surface-*`, `--app-border-*`, `--app-text-*`, `--app-hover-bg`, and `--app-selected-bg`.
- Do not add page-specific `[data-theme='dark']` CSS blocks for new UI. New components should inherit light/dark mode automatically through the shared tokens.
